...
async def build_article_urls(keywords):
    ...
    for keyword in keywords:
        ...
        for page in range(1,num_pages+1):
            task = asyncio.get_event_loop().create_task(get_pmids(page, keyword))
            tasks.append(task)
...
async def get_article_data(urls):
    ...
    for url in urls:
        if url not in scraped_urls:
            task = asyncio.get_event_loop().create_task(extract_by_article(url))
            tasks.append(task)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start', type=int, default=2012, help='Specify start year for publication date range to scrape. Default = 2016')
    parser.add_argument('--stop', type=int, default=2017, help='Specify stop year for publication date range to scrape. Default = 2021')
    parser.add_argument('--output', type=str, default='article_data.csv',help='Choose output file name. Default = "article_data.csv".')
    ...
    semaphore = asyncio.BoundedSemaphore(400)
    ...
    articles_df = pd.DataFrame(articles_data, columns=['abstract','keywords', 'url'])
